...
## Estado del repo: siete changes del MVP archivados (`data-model`, `auth-pin`, `catalogo`, `venta`, `dashboard`, `color-palette-assistant`, `unpair-device`) — LIVE en producción (`antimahue.com`)
...
**Ya hay tests** (Vitest): `pnpm test` corre la suite (`385 passed | 7 skipped` por default — los 7 `it.skip` son la batería RLS multi-rol, que solo corre contra un stack Supabase local disponible; `RUN_LOCAL_RLS_BATTERY=1 pnpm test` la fuerza → 392/392). También existen `pnpm lint` (ESLint) y `pnpm format` / `pnpm format:check` (Prettier). CI vive en `.github/workflows/ci.yml` (GitHub Actions): corre lint + format:check + typecheck + test + build en cada PR contra `main` y en push a `main` — introducido en la fase toolchain de `auth-pin` (T-0.4). El change `setup-stack` está **cerrado y archivado** (apply 48/48 + `sdd-archive` ejecutados el 2026-06-22, ver `diario/2026-06-22.md`): vive en `openspec/changes/archive/2026-06-22-setup-stack/` y su spec consolidada quedó en `openspec/specs/setup-stack/spec.md`.
...
El cambio `catalogo` está **mergeado a `main` y archivado** (apply completo, verify PASS — lint/format/typecheck/test/build verdes + verificación manual DD-1..DD-7 —, `sdd-archive` 2026-07-17, PR #41): primera feature de negocio usable del MVP. Listado con búsqueda debounced (300 ms) y filtros (`/catalogo`), detalle con vista diferenciada admin/empleado (el embed `producto_costos` degrada a `null` para empleados — el costo nunca llega al cliente), creación/edición admin-only (`/catalogo/new`, `/catalogo/:id/edit`, gateadas con `<RequireAdmin>` a nivel ruta y por los RPCs `crear_producto`/`actualizar_producto` a nivel DB), y escáner de códigos de barras (`/escaner`) con Barcode Detection API nativa + fallback manual de SKU. Sin migraciones nuevas: reuso puro del schema de `data-model`. Componentes compartidos nuevos: `BottomNav`, `ScreenHeader`, `SearchInput`, `FilterChips`, `ProductCard`, `StockBadge`, `Stepper`. Decisiones diferidas: imágenes por URL externa/placeholder — Storage queda para cuando Angélica valide fotos propias (D3) — y sin librería externa de escaneo (D2). Vive en `openspec/changes/archive/2026-07-17-catalogo/` y el spec consolidado quedó en `openspec/specs/catalogo/spec.md`.

El cambio `venta` está **mergeado a `main` y archivado** (apply completo, verify PASS — 233 tests, 5 gates green —, `sdd-archive` 2026-08-13, PRs #44/#45): flujo de venta teléfono-first. Carrito con selección rápida de productos, confirmación, deshacer (tolerancia a errores), ticket térmico visual y cierre de venta contra el schema de `data-model`. Cierre de issues #6 y #9. Vive en `openspec/changes/archive/2026-08-13-venta/` y el spec consolidado quedó en `openspec/specs/venta/spec.md`.

El cambio `dashboard` está **mergeado a `main`, LIVE y archivado** (apply 18/18, verify PASS — 271 tests, 5 gates green —, `sdd-archive` 2026-08-18, PRs #47/#48): pantalla principal que Angélica ve al desbloquear. Resumen del día (ventas, ítems, desglose por medio de pago), alertas de stock (top 10, mínimo efectivo resuelto en SQL con `COALESCE(productos.stock_minimo, configuracion.stock_minimo_default)`), valor de inventario a costo **solo admin** (empleados reciben `valor_inventario: null` desde la RPC, nunca desde el cliente) y búsqueda rápida que navega al catálogo. Read-only: sin edición de stock ni notificaciones reales. Una migración nueva: `20260818000000_resumen_dashboard_rpc.sql` (RPC `resumen_dashboard()`, SECURITY DEFINER, verificada en prod el 2026-09-08). Código en `src/features/dashboard/`. Vive en `openspec/changes/archive/2026-08-18-dashboard/` y el spec consolidado quedó en `openspec/specs/dashboard/spec.md` (dominio nuevo). 

El cambio `color-palette-assistant` está **LIVE y archivado** (apply en 3 PRs apilados #50/#51/#52 + corrección #54, verify PASS WITH WARNINGS — 0 CRITICAL, 2 WARNING —, `sdd-archive` 2026-09-09): asistente de combinación de colores en `/paleta`. Semilla (ovillos con `color_hex`, búsqueda por nombre y nombre de color sin acentos) → regla (análogos ±30°, complementario 180°, triádico 120°/240°) → sugerencias por distancia HSL con hue circular, sin stock **marcado, no oculto** → armar y reordenar paleta → compartir por WhatsApp (nombres y colores, sin precios) → "Anotar encargo" en `pedidos_pendientes` (**solo admin**, RLS + gating en UI). Migración `20260908000000_color_palette_hsl.sql`: columnas `color_h/s/l` con CHECKs, helper interno `hex_to_hsl` (sin EXECUTE para roles API), RPCs de producto recalculan HSL server-side, tabla `pedidos_pendientes`. Quinta pestaña "Paleta" en `BottomNav` (desvío del handoff de 4 pestañas, documentado). Código en `src/features/paleta/`. Vive en `openspec/changes/archive/2026-09-09-color-palette-assistant/`; specs consolidados en `openspec/specs/color-palette/spec.md` (dominio nuevo) y `openspec/specs/catalogo/spec.md`. Residuales: prod tiene pocos productos con color para ejercer el ranking; el embed `producto_costos` en `paletaApi` no se usa.

El cambio `unpair-device` está **LIVE y archivado** (PR #53, verify PASS WITH WARNINGS — 0 CRITICAL, 3 WARNING —, `sdd-archive` 2026-09-09): en la pantalla del PIN, bajo `correo · rol`, el enlace "¿No eres tú? Desvincular este teléfono" con confirmación en línea (nota de consecuencia + "Sí, desvincular" / "Cancelar"; sin modal porque no hay deshacer: volver a vincular exige la contraseña). Borra el registro local del vault, resetea `$lock` (`resetLock()` nuevo en `src/stores/lock.ts`) y relee el vault: sin perfiles → `/pair`, uno → autoselección, dos o más → selector; "Cambiar de usuario" solo con dos o más perfiles. Sin PIN, sin red, sin tocar el wipe por 9 fallos. Guardas: acciones deshabilitadas con 4 dígitos ingresados (un PIN erróneo en vuelo podía resucitar el registro borrado) y panel keyed por usuario. Specs consolidados en `openspec/specs/auth/spec.md` (REQ-AUTH-5..10). Próximo change del roadmap: `proveedores-dte`.

**Fixes post-archive del 2026-09-09** (todos con causa raíz reproducida en prod dentro de transacciones revertidas): edición de producto fallaba para cualquier producto con stock porque el formulario mandaba `stock` en `p_producto` y el RPC lo rechaza (#55, ahora viaja `p_stock_delta`); el escáner nunca tuvo cámara porque `public/_headers` enviaba `Permissions-Policy: camera=()` (#56, ahora `camera=(self)`); el dashboard nunca cargó porque `resumen_dashboard` sumaba `total` sobre una subconsulta que no lo expone (#57, migración `20260909000000_resumen_dashboard_fix`). Lección incorporada al proceso: **toda función SQL que entregue un change se ejecuta contra el stack local con datos sembrados en `verify`**, no solo tests de frontend con el RPC simulado.

**Gotcha operativo (2026-09-08):** el proyecto Supabase es free tier y se **auto-pausa tras ~7 días sin uso** (`status: INACTIVE`). El frontend sigue dando HTTP 200 (bundle estático) pero todo login/dato falla. Antes de tocar la DB, chequear `get_project.status`; mientras está en `COMING_UP`, `list_migrations` devuelve lista vacía, no error.
...
| --- | --- | --- |
| _(ninguno)_ | — | `active_changes: []` en `project.yaml`. Próximo del roadmap: `proveedores-dte`. |
...
- `catalogo` (2026-07-17) — CRUD de productos + búsqueda + detalle por rol + escáner de códigos de barras (PR #41)
- `venta` (2026-08-13) — flujo de venta con carrito, confirmación, deshacer y ticket térmico (PRs #44/#45)
- `dashboard` (2026-08-18) — pantalla principal: resumen del día, alertas de stock, valor de inventario admin-only, búsqueda rápida (PRs #47/#48)
- `color-palette-assistant` (2026-09-09) — asistente de combinación de colores `/paleta` con HSL server-side y encargos pendientes (PRs #50/#51/#52/#54)
- `unpair-device` (2026-09-09) — desvincular teléfono y cambiar de usuario desde la pantalla del PIN (PR #53)
...
- **Timestamps de migración:** una migración con versión anterior a la última aplicada en prod la salta `db push` (la integración). Al escribir un change, fechar la migración con el día real del apply, no con la fecha del design.
- **Supabase free tier se auto-pausa** tras ~7 días sin uso (`status: INACTIVE`). El frontend sigue dando 200 pero todo login/dato falla. Chequear `get_project.status` al arrancar; mientras está en `COMING_UP`, `list_migrations` devuelve lista vacía, no error.
- **Rol admin vive en dos lugares:** `profiles.rol` (autorización real, `is_admin()`) y `raw_app_meta_data.rol` (etiqueta del selector del PIN, leída al vincular). Al provisionar un admin hay que setear ambos; solo el segundo se fija con service_role.
- **Copy de UI en tuteo neutro chileno** ("Elige", "Agrega", "Busca"), nunca voseo. Los docs y este archivo sí van en rioplatense.
- **CI solo corre en PRs contra `main`:** en cadenas de PRs apilados, los PRs contra ramas intermedias no tienen CI y retargetear no lo dispara. Abrir cada PR contra `main` después de mergear el padre.
- **Cabeceras de seguridad (`public/_headers`)** se escribieron antes de que existieran las features: cualquier change que use APIs del dispositivo (cámara, geolocalización) tiene que revisar `Permissions-Policy` y CSP en su design.
