...
## Estado del repo: tres changes del MVP archivados (`data-model`, `auth-pin`, `catalogo`) — LIVE en producción (`antimahue.com`)
...
**Ya hay tests** (Vitest): `pnpm test` corre la suite (`166 passed | 7 skipped` por default — los 7 `it.skip` son la batería RLS multi-rol, que solo corre contra un stack Supabase local disponible; `RUN_LOCAL_RLS_BATTERY=1 pnpm test` la fuerza → 173/173). También existen `pnpm lint` (ESLint) y `pnpm format` / `pnpm format:check` (Prettier). CI vive en `.github/workflows/ci.yml` (GitHub Actions): corre lint + format:check + typecheck + test + build en cada PR contra `main` y en push a `main` — introducido en la fase toolchain de `auth-pin` (T-0.4). El change `setup-stack` está **cerrado y archivado** (apply 48/48 + `sdd-archive` ejecutados el 2026-06-22, ver `diario/2026-06-22.md`): vive en `openspec/changes/archive/2026-06-22-setup-stack/` y su spec consolidada quedó en `openspec/specs/setup-stack/spec.md`.
