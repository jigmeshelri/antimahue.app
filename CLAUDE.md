...
## Estado del repo: cinco changes del MVP archivados (`data-model`, `auth-pin`, `catalogo`, `venta`, `dashboard`) — LIVE en producción (`antimahue.com`)
...
**Ya hay tests** (Vitest): `pnpm test` corre la suite (`271 passed | 7 skipped` por default — los 7 `it.skip` son la batería RLS multi-rol, que solo corre contra un stack Supabase local disponible; `RUN_LOCAL_RLS_BATTERY=1 pnpm test` la fuerza → 278/278). También existen `pnpm lint` (ESLint) y `pnpm format` / `pnpm format:check` (Prettier). CI vive en `.github/workflows/ci.yml` (GitHub Actions): corre lint + format:check + typecheck + test + build en cada PR contra `main` y en push a `main` — introducido en la fase toolchain de `auth-pin` (T-0.4). El change `setup-stack` está **cerrado y archivado** (apply 48/48 + `sdd-archive` ejecutados el 2026-06-22, ver `diario/2026-06-22.md`): vive en `openspec/changes/archive/2026-06-22-setup-stack/` y su spec consolidada quedó en `openspec/specs/setup-stack/spec.md`.
...
El cambio `dashboard` está **mergeado a `main`, LIVE y archivado** (apply 18/18, verify PASS — 271 tests, 5 gates green —, `sdd-archive` 2026-08-18, PRs #47/#48): pantalla principal que Angélica ve al desbloquear. Resumen del día (ventas, ítems, desglose por medio de pago), alertas de stock (top 10, mínimo efectivo resuelto en SQL con `COALESCE(productos.stock_minimo, configuracion.stock_minimo_default)`), valor de inventario a costo **solo admin** (empleados reciben `valor_inventario: null` desde la RPC, nunca desde el cliente) y búsqueda rápida que navega al catálogo. Read-only: sin edición de stock ni notificaciones reales. Una migración nueva: `20260818000000_resumen_dashboard_rpc.sql` (RPC `resumen_dashboard()`, SECURITY DEFINER, verificada en prod el 2026-09-08). Código en `src/features/dashboard/`. Vive en `openspec/changes/archive/2026-08-18-dashboard/` y el spec consolidado quedó en `openspec/specs/dashboard/spec.md` (dominio nuevo). Próximo change del roadmap: `proveedores-dte`; `color-palette-assistant` sigue activo.
...
| --- | --- | --- |
| `color-palette-assistant` | `tasks` completado (22 tareas, 5 fases), `apply` pending | Asistente de armonía cromática que mapea teoría de color contra stock real. |
